...
const search = async () => {
  const res = await request.post("/user/searchFriend", {
...
onMounted(async () => {
  const resLoginUser = await getCurrent();
  status.value.loginUser = resLoginUser.data;
  const resFriends = await request.get("/user/friends")
  status.value.friends = resFriends.data;
})
